import { Briefcase, Building2, Calendar, Edit, FileText, Link, Mail, Trash2 } from "lucide-react";
import React, { useEffect, useRef } from "react";
import { useSearchParams } from "react-router-dom";
import type { IApplication } from "../../../interfaces/types";

interface EditingRowAppProps {
    app: IApplication;
...
const EditingRowApp: React.FC<EditingRowAppProps> = React.memo(({
    app,
    formatDate,
    statusColors,
    statusOptions,
    ...
    const handleFollowUpClick = React.useCallback((e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        onFollowUp();
    }, [onFollowUp]);
                ...
                <td className="px-6 py-4">
                    {app.contact_email ? (
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-gray-900">{app.contact_email}</span>
                        </div>
                    ) : (
                        <span className="text-gray-400">-</span>
                    )}
...
EditingRowApp.displayName = 'EditingRowApp';

export default EditingRowApp;